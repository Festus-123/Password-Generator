...
import { generatePassword, type UserDetails, type passwordConfig, } from '../services/generator';
...
const Formulator = () => {
  ...
  // console.log(generatePassword(userDetails, config, 10))
  ...
  return (
    <div className='flex flex-col md:flex-row md:justify-evenly lg:justify-evenly lg:flex-row p-4 lg:p-6 gap-2 lg:gap-4'>
        ...
        {/* 
        <select name="" id="" value={config.type} onChange={(e) => setConfig({...config, type: e.target.value})} className="bg-[#80808022] py-2 px-4 lg:p-3 lg:px-4 w-full rounded-2xl text-black">
          {['password type','numeric', 'alphabetical', 'mixed'].map((char, index) => (
            <option key={index} value={char}>{char}</option>
          ))}
        </select> */}

        <select name="" id="" value={config.lenght} onChange={e => setConfig({...config, lenght: Number(e.target.value)})} className="bg-[#80808022] py-2 px-4 lg:p-3 lg:px-4 w-full rounded-2xl text-black">
